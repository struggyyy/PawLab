...
export class TaskService {
  ...
  static async createTask(
    ...
    try {
      ...
      if (error) {
        console.error(
          "Error creating task (Supabase error object):",
  ...
  static async updateTask(
    taskId: string,
    updatedTaskData: Partial<Omit<Task, "id" | "created_at" | "project_id">>
  ): Promise<Task | null> {
    try {
      const { data, error } = await supabase
        .from("tasks")
  ...
  static async updateTaskHours(
    ...
    const currentWorkedHours =
      typeof task.worked_hours === "number" ? task.worked_hours : 0;
  ...
  // For debugging - clear all tasks from a specific project
  static async clearTasksByProjectId(projectId: string): Promise<boolean> {
    console.warn(
      `Attempting to delete all tasks for project ${projectId}. This is a destructive operation.`
    );
    try {
